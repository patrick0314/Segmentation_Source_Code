...
class regionGrow():
    def __init__(self, im_path, out_path, th):
    ...
    def Save(self):
        cv2.imwrite(self.output, self.SEGS)
...
if __name__ == '__main__':
    input_folder = 'pics'
    output_folder = 'results_manual'
    filenames = [f for f in os.listdir(input_folder) if os.path.isfile(os.path.join(input_folder, f))]
    for filename in filenames:
        print('=== Region Growing on {}'.format(filename))
        seeds=[]
        exemple = regionGrow(os.path.join(input_folder, filename), os.path.join(output_folder, filename), 10) # threshold can be 10/12/15/...
        cv2.namedWindow('image')
        cv2.setMouseCallback('image',Test_Affiche)
        cv2.imshow('image', cv2.imread(os.path.join(input_folder, filename), 1))
        cv2.waitKey(0)
